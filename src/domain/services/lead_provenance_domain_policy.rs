//! Domain policy for LeadProvenance aggregate
//!
...
/// Domain policy for LeadProvenance — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<LeadProvenance>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type LeadProvenanceDomainPolicy = PermitAllPolicy<LeadProvenance>;
