//! Domain policy for Stage aggregate
//!
...
/// Domain policy for Stage — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Stage>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type StageDomainPolicy = PermitAllPolicy<Stage>;
