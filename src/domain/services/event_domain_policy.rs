//! Domain policy for Event aggregate
//!
...
/// Domain policy for Event — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Event>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type EventDomainPolicy = PermitAllPolicy<Event>;
