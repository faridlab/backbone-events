//! Domain policy for Ticket aggregate
//!
...
/// Domain policy for Ticket — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Ticket>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type TicketDomainPolicy = PermitAllPolicy<Ticket>;
