//! Domain policy for LeadRequest aggregate
//!
...
/// Domain policy for LeadRequest — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<LeadRequest>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type LeadRequestDomainPolicy = PermitAllPolicy<LeadRequest>;
