//! Domain policy for Booth aggregate
//!
...
/// Domain policy for Booth — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Booth>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type BoothDomainPolicy = PermitAllPolicy<Booth>;
