//! Domain policy for LeadRulePredicate aggregate
//!
...
/// Domain policy for LeadRulePredicate — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<LeadRulePredicate>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type LeadRulePredicateDomainPolicy = PermitAllPolicy<LeadRulePredicate>;
