-- Ticket Seed Data
-- Generated by metaphor-schema
--
-- This file contains initial seed data for the event.tickets table.
-- Customize this file to add your initial data.

-- INSERT INTO event.tickets (
--     id,
--     event_id,
--     name,
--     seats_max,
--     seats_max_per_order,
--     start_sale_datetime,
--     end_sale_datetime,
--     metadata
-- ) VALUES
-- (
--     -- Add values here
-- );

-- Example: Insert reference data
-- Uncomment and modify the following:

-- INSERT INTO event.tickets (id, code, name, is_active, created_at, updated_at) VALUES
--     (gen_random_uuid(), 'SAMPLE1', 'Sample 1', true, NOW(), NOW()),
--     (gen_random_uuid(), 'SAMPLE2', 'Sample 2', true, NOW(), NOW());

-- <<< CUSTOM SEED DATA >>>
-- Add your custom seed data below

