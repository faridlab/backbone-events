...
metaphor migration generate Invoice events
metaphor migration run
...
(`events` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed events          # run Rust seeders
metaphor migration generate-seeds events  # emit SQL seed files
```
