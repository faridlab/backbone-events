...
|-------------|-------|-----------------|
| `events` | Your module name (lowercase, e.g. `payments`) | `index.model.yaml`, `metaphor.codegen.yaml`, `business-flows/README.md`, `hooks`, `workflows` |
...
`events` is the token the skeleton stamps to your real module name; `Example` stays concrete
...
- [ ] Replace `events` with your module name everywhere it appears (the skeleton's
...
| [05-maintainer-guide](05-maintainer-guide.md) | **Keep verbatim** — the regen workflow and rules are module-agnostic. Commands already use `events`. |
