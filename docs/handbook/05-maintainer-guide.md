...
metaphor make entity Vendor --module events
...
metaphor schema schema validate events

# 3. Generate all artifacts (entity, DTOs, repo, service, handler, routes).
metaphor schema schema generate events --target all --force

# 4. Generate the migration for the new entity.
metaphor migration generate Vendor events
...
> `events` is your module name (auto-detected from the current directory when omitted).
...
2. `metaphor schema schema validate events`.
3. Generate a migration for the change:
   `metaphor migration generate <Entity> events` (or, for a schema-diff-driven migration against
   a live DB, `metaphor schema schema migration events --database-url …`).
4. Regenerate code: `metaphor schema schema generate events --target all --force`.
