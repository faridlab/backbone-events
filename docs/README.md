...
The documentation set for the **`events`** Backbone Framework domain module.

> **This handbook is stamped from the [module skeleton][skel].** In the unmodified skeleton,
> `events` is `backbone-module-skeleton` (v0.1.3) and the reference entity is `Example` — one
...
0. [Adapting this handbook to your module](handbook/00-adapting-to-your-module.md) — *Maintainer.* Read first when stamping a new module: what `events` means, which pages to keep verbatim vs. fill in.
